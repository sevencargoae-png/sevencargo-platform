(function () {
  ...
  PAGES.account = async (el) => {
    el.innerHTML = `<div class="card" style="max-width:480px"><h3>${t('acc_change_pw')}</h3><form id="pw-form">
      ...
      ${me.role === 'admin' ? `<div class="card" style="max-width:480px;margin-top:16px"><h3>💾 ${t('bk_title')}</h3><p class="small muted">${t('bk_desc')}</p><a class="btn btn-primary" href="/api/admin/backup" download>${t('bk_btn')}</a></div>` : ''}`;
